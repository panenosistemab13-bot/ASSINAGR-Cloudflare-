...
const VisualizadorDeMapa: React.FC<VisualizadorDeMapaProps> = ({ 
  ...
  useEffect(() => {
    const loadAllData = async () => {
      try {
        setLoading(true);
        // Busca configurações globais e dados específicos da rota em paralelo
        const [genTerms, rData] = await Promise.all([
          api.settings.getTerms(),
          api.routes.get(currentRouteId)
        ]);
        
        if (genTerms) setTerms(genTerms);
